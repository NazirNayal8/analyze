...
class OODAnalyzer:
    ...
    def evaluate_mIoU(
        ...
        **kwargs
        ...
        mIoU_metric = JaccardIndex(
            num_classes=num_classes, ignore_index=ignore_index).to(device)
    ...
    def extract_anomaly_score(self, loader, dataset_name, device=torch.device('cpu'), skip=1, **kwargs):
        ...
        df["logits"] = np.concatenate(
            [anomaly_score_ood, anomaly_score_id]).astype(np.float32)
        df["labels"] = labels
    ...
    def extract_anomaly_score_id(
        ...
        **kwargs
        ...
        for x, y in tqdm(loader, desc='Loader for an ID Dataset'):
            ...
            anomaly_score_id.extend(
                anomaly_score.reshape(-1).cpu().tolist()[::id_skip])
            ...
            for c in range(num_classes):
                ...
                if num_pixels == 0:
                    ...
                    anomaly_score_per_class[c].extend(
                        anomaly_score[mask].cpu().tolist())
                else:
                    anomaly_score_per_class[c].extend(
                        anomaly_score[mask].cpu().tolist()[::id_skip])

        df = pd.DataFrame()
        df["logits"] = np.array(anomaly_score_id[::skip]).astype(np.float32)
        df["labels"] = np.array(
            [f'{dataset_name}_id'] * len(anomaly_score_id[::skip]))
        ...
        for c in range(num_classes):
            ...
            if verbose:
                print("{:20} {:20}".format(class_name,
                      len(anomaly_score_per_class[c])))
            ...
            class_df["labels"] = np.array(
                [f'{dataset_name}_{class_name}'] * len(anomaly_score_per_class[c][::sk]))
