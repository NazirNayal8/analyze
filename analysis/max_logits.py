...
class MaxLogitsAnalyzer:
    ...
    def extract_max_logits_ood(self, loader, dataset_name, device=torch.device('cpu'), skip=1, **kwargs):
        ...
        for x, y in tqdm(loader, desc="Loader for an OOD Dataset"):
            ...
            # eliminate background predicted logi
        ...
        df["logits"] = np.concatenate(
            [max_logits_ood, max_logits_id]).astype(np.float32)
        df["labels"] = labels
    ...
    def extract_max_logits_id(
        ...
        **kwargs
        ...
        for x, y in tqdm(loader, desc='Loader for an ID Dataset'):
            ...
            max_logits_id.extend(
                max_logit.reshape(-1).cpu().tolist()[::id_skip])
            ...
            for c in range(num_classes):
                ...
                if num_pixels == 0:
                    ...
                    max_logits_per_class[c].extend(
                        max_logit[mask].cpu().tolist())
                else:
                    max_logits_per_class[c].extend(
                        max_logit[mask].cpu().tolist()[::id_skip])

        df = pd.DataFrame()
        df["logits"] = np.array(max_logits_id[::skip]).astype(np.float32)
        df["labels"] = np.array(
            [f'{dataset_name}_id'] * len(max_logits_id[::skip]))
        ...
        for c in range(num_classes):
            ...
            if verbose:
                print("{:20} {:20}".format(
                    class_name, len(max_logits_per_class[c])))
            ...
            class_df["labels"] = np.array(
                [f'{dataset_name}_{class_name}'] * len(max_logits_per_class[c][::sk]))
    ...
    def visualize_kde(self, df, bw_adjust=1, figsize=(8, 5), chosen_cols=None):
        ...
        sns.kdeplot(data=vis_df, x='logits', hue='labels',
                    fill=True, common_norm=False, bw_adjust=bw_adjust)
        plt.show()
...
class OODEvaluator:
    ...
    def compute_max_logit_scores(
        ...
        for x, y in tqdm(loader, desc="Dataset Iteration"):
            ...
            max_logit, preds = logits[:, :19, :, :].max(
                dim=1)  # eliminate background predicted logit

            if return_preds:
                predictions.extend([preds.cpu().numpy()])

            if use_gaussian_smoothing:
                max_logit = gaussian_smoothing(
                    max_logit.unsqueeze(0)).squeeze(0)
    ...
    def log_to_wandb(
        ...
        self.log_anomaly_maps(imgs, anomaly_score, ood_gts,
                              logger=logger, upper_limit=upper_limit)
        self.log_id_maps(imgs, preds, class_names, y=None,
                         logger=logger, upper_limit=upper_limit)
    ...
    def get_imgs(self, dataset, image_mean, image_std, out_type=np.uint8, upper_limit=2000):
        ...
        for i in range(len(dataset)):
            if i >= upper_limit:
                break
            x, _ = dataset[i]
            imgs.extend([unnormalize_tensor(x.float(), np.array(
                image_mean), np.array(image_std)).cpu().permute(1, 2, 0).numpy()])
